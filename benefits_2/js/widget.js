//ЧТО НУЖНО УЧЕСТЬ
// 1) для формы нужно будет подтянуть id (this._formId). Данила показывал, что определял id формы в своем скрипте. Оттуда его можно будет взять
// 2) для запросов на сервер инкоманд нужно будет в заголовки запроса подставить Liferay.authTocken. Ниже я добавил эту переменную. Нужно будет снять кавычки
// 3) скрипт взаимодействует с версткой формы через атрибуты data-field-name. В примерах верстки они были. Нужно просто чекнуть, что это правило распространяется на все поля формы
// 4) в самом конце скрипта определил кнопки согласия и отказа через дата-атрибуты. Тут либо добавлять эти дата-атрибуты в ftl, либо искать через уже заданыне в ftl атрибуты
// 5) также нужно проверить запросы на получение файла на соответсвие договоренностям, так как у меня была не вся инфа
// 6) при интеграции убрать из запросов моки
