...
// 2) для запросов на сервер инкоманд нужно будет в заголовки запроса подставлять Liferay.authTocken. Ниже я добавил эту переменную. Нужно будет снять кавычки
// 3) скрипт взаимодействует с версткой формы через атрибуты data-field-name. В примерах вертки они были. Нужно просто чекнуть, что это правило распространяется на все поля формы
// 4) в самом конце скрипта определил кнопки согласия и отказа через дата-атрибуты. Тут либо добавлять дата-атрибуты в ftl, либо искать через уже заданыне в ftl атрибуты
